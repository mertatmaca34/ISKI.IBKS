...
public class AnalogSensorService : IAnalogSensorService
{
    ...
    private readonly ISelectedSensorsProvider _selectedSensorsProvider;
    ...
    public AnalogSensorService(
        ...
        ISelectedSensorsProvider selectedSensorsProvider)
    {
        ...
        _selectedSensorsProvider = selectedSensorsProvider ?? throw new ArgumentNullException(nameof(selectedSensorsProvider));
    }
    ...
    public async Task<IDataResult<IReadOnlyList<ChannelReadingDto>>> GetChannelsAsync(Guid stationId, CancellationToken cancellationToken = default)
    {
        ...
        var selectedSensors = await _selectedSensorsProvider.GetSelectedSensorsAsync(cancellationToken);
        ...
        foreach (var map in ordered)
        {
            ...
            if (selectedSensors.Count > 0 && !selectedSensors.Any(s => 
                s.Equals(map.Key, StringComparison.OrdinalIgnoreCase)))
                continue;
