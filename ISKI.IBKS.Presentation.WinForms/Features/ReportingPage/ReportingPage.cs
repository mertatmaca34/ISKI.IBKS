    ...
    public partial class ReportingPage : UserControl
    {
        ...
        private async void ButtonGenerate_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (DataGridViewDatas != null)
                {
                    ...
                    if (DataGridViewDatas.Columns.Contains("Veri Tarihi"))
                    {
                        DataGridViewDatas.Columns["Veri Tarihi"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
                    }
        ...
        private async Task<DataTable> GenerateReportDataAsync(string reportType, DateTime startDate, DateTime endDate, bool sortNewestFirst)
        {
            ...
            switch (reportType)
            {
                ...
                case "Ölçüm":
                    var stationSettings = await dbContext.StationSettings.AsNoTracking().FirstOrDefaultAsync();
                    ...
                    // Dinamik Sensör Sütunları
                    foreach (var sensorKey in selectedSensors)
                    {
                        dt.Columns.Add(GetFriendlyName(sensorKey));
                    }
                    ...
                    foreach (var m in measurements)
                    {
                        ...
                        foreach (var sensorKey in selectedSensors)
                        {
                            row[GetFriendlyName(sensorKey)] = GetSensorValue(m, sensorKey);
                        }
        ...
        private string GetFriendlyName(string sensorKey)
        {
            return sensorKey switch
            {
                ...
                "DesarjDebi" => "Deşarj Debisi (m³/h)",
                "HariciDebi" => "Harici Debi (m³/h)",
                "HariciDebi2" => "Harici Debi 2 (m³/h)",
                _ => sensorKey
            };
        }

        private string GetSensorValue(SensorData m, string sensorKey)
        {
            return sensorKey switch
            {
                ...
                "DesarjDebi" => m.DesarjDebi?.ToString("F1") ?? "-",
                "HariciDebi" => m.HariciDebi?.ToString("F1") ?? "-",
                "HariciDebi2" => m.HariciDebi2?.ToString("F1") ?? "-",
                _ => "-"
            };
        }
