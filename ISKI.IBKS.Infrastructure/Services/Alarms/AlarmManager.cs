...
public class AlarmManager : IAlarmManager
{
    ...
    private async Task HandleActiveAlarmAsync(AlarmDefinition def, Guid stationId, string message, double? triggerValue, CancellationToken ct)
    {
        ...
        // 2. Per-Alarm Cooldown Check
        var cooldown = TimeSpan.FromMinutes(def.CooldownMinutes);
        if (now - state.LastNotificationSentAt > cooldown)
        {
        ...
        var alarmEvent = new AlarmEvent(
            ...
            AlarmPriority.Medium // Default priority for backward compatibility
        )
        {
            Id = Guid.NewGuid()
        ...
        if (shouldSendNotification)
        {
            try 
            {
                // 4. Get Subscribers - no priority filtering, just active users with email enabled
                ...
                if (subscribers.Any())
                {
                    ...
                    if (sentCount > 0)
                    {
                        ...
                        _logger.LogInformation("Alarm bildirimi gönderildi: {AlarmName} - {Count} kullanıcı (Cooldown: {Cooldown} dk)", 
                            def.Name, sentCount, def.CooldownMinutes);
                    }
                    else
                    {
        ...
        {
             _logger.LogDebug("Alarm aktif fakat cooldown içinde: {AlarmName} (Cooldown: {Cooldown} dk)", def.Name, def.CooldownMinutes);
        }
